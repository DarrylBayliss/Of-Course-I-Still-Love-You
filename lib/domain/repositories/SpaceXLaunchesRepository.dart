...
class SpaceXLaunchesRepository extends LaunchesRepository {
  ...
  Future<List<Rocket>> getRockets() async {
    return spaceXApi.getRockets().then((rocketDTOList) => rocketDTOList.map((rocketDTO) => Rocket.fromDTO(rocketDTO)).toList());
  }

  @override
  Future<List<RocketLaunch>> getLaunchesForRocket(String rocketId) async {
    return spaceXApi.getLaunchesForRocket(rocketId).then((rocketLaunchDTOList) => rocketLaunchDTOList.map((rocketLaunchDTO) => RocketLaunch.fromDTO(rocketLaunchDTO)).toList());
  }
}
